...
def encrypt_with_aes(input_string, password, salt):
    key = generate_aes_key(password, salt)  # Corrected line
    f = Fernet(key)  # Corrected line
    encrypted_data = f.encrypt(input_string.encode('utf-8'))  # call the Fernet encrypt method #Corrected line
...
def decrypt_with_aes(encrypted_data, password, salt):
    key = generate_aes_key(password, salt)  # Corrected line
    f = Fernet(key)  # Corrected line
    decrypted_data = f.decrypt(encrypted_data)  # call the Fernet decrypt method #Corrected line
    return decrypted_data.decode('utf-8')


salt = b'Tandon'  # Remember it should be a byte-object # [cite: 25]
password = "it2284@nyu.edu"  # [cite: 26]
input_string = "AlwaysWatching"  # [cite: 26]
...
def run_dns_server():
    ...
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Research this #Corrected line
    server_socket.bind(('127.0.0.1', 53))  # Corrected line
    ...
    while True:
        try:
            ...
            request = dns.message.from_wire(data)  # Corrected line
            # Create a response message using the `dns.message.make_response` method
            response = dns.message.make_response(request)  # Corrected line

            # Get the question from the request
            question = request.question[0]  # Corrected line
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX:
                    ...
                    mname, rname, serial, refresh, retry, expire, minimum = answer_data  # What is the record format? See dns_records dictionary. Assume we handle @, Class, TTL elsewhere. Do some research on SOA Records #Corrected line
                    rdata = SOA(dns.rdataclass.IN, dns.rdatatype.SOA, mname, rname, serial, refresh, retry, expire,
                                minimum)  # follow format from previous line #Corrected line
            ...
            server_socket.sendto(response.to_wire(), addr)  # Corrected line
